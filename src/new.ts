 ...
 *
 * Writes src/<singular>.ts with every blank marked TODO, and adds the import and
 * registration lines to src/api.ts. The result does NOT pass
...
const file = join(root, "src", `${singular}.ts`);
if (existsSync(file)) {
...
writeFileSync(
  file,
  `import { entity, resource, action, str, int, num, bool, json, list, obj, ref } from "./define.js";
...
const indexPath = join(root, "src", "api.ts");
...
console.log(`Created ${file.replace(root + "/", "")} and registered it in src/api.ts.
