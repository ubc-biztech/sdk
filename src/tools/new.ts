/**
 * `npm run new -- <singular> [plural]` — scaffold a resource file and register it.
 *
 * Writes src/ontology/entities/<singular>.ts with every blank marked TODO, and adds the
 * import and registration lines to src/ontology/index.ts. The result does NOT pass
 * `npm run check` until every TODO is replaced: the validator rejects empty descriptions,
 * so a half-finished scaffold cannot be generated by accident.
 */
...
const file = join(root, "src", "ontology", "entities", `${singular}.ts`);
if (existsSync(file)) {
...
const collection = plural
  ? `  collection: {
    list: action({
      ...
      auth: "public",   // one of: public, authenticated, member, admin  (src/ontology/roles.ts)
...
writeFileSync(
  file,
  `import { entity, resource, action, str, int, num, bool, json, list, obj, ref } from "../dsl.js";
...
const indexPath = join(root, "src", "ontology", "index.ts");
let index = readFileSync(indexPath, "utf8");
const importLine = `import { ${Entity}, ${plural ?? singular} } from "./entities/${singular}.js";\n`;
...
console.log(`Created ${file.replace(root + "/", "")} and registered it in src/ontology/index.ts.
